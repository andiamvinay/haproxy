...
local function fetch_jwt_token(conf, ctx)
    ...
    local precookie = sub_str(val, 1, 7)
    if precookie == 'Bearer ' or precookie == 'bearer ' then
        return sub_str(val, 8)
    end
    local precookiea = sub_str(val, 1, 9)
    if precookiea == 'Bearer%20' or precookiea == 'Bearer%20' then
        return sub_str(val, 10)
    end
    return val
end
